...
enum { ntrial = 1000, timeout_usec = 10000000, nbytes = 32 };
...
static void
communicate(nrf_t *server, nrf_t *client, uint32_t addr, int verbose_p, int order) {
    unsigned ntimeout = 0, npackets = 0;
...
void notmain(void) {
    ...
    communicate(s, c, client_addr_other, 1, 0);

    nrf_stat_print(c, "new server: done with one-way test");
    nrf_stat_print(s, "new client: done with one-way test");
}
